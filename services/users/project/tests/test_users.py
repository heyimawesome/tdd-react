...
class TestUserService(BaseTestCase):
    ...
    def test_add_user(self):
        ...
        add_user('test', 'test@test.com', 'password', admin=True)
        ...
        with self.client:
            ...
            self.assertIn('james@email.com was added!', data['message'])
            self.assertIn('success', data['status'])
    ...
    def test_add_user_invalid_json_keys(self):
        ...
        with self.client:
            response = self.client.post(
                '/users',
                data=json.dumps({'email': 'james@email.com'}),
                content_type='application/json',
                headers={'Authorization': f'Bearer {token}'}
            )
            data = json.loads(response.data.decode())
    ...
    def test_single_user(self):
        ...
        user = add_user('james', 'james@email.com', 'password')
        with self.client:
            response = self.client.get(f'/users/{user.id}')
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 200)
            self.assertIn('james', data['data']['username'])
            self.assertIn('james@email.com', data['data']['email'])
            self.assertIn('success', data['status'])
    ...
    def test_all_users(self):
        ...
        add_user('james', 'james@email.com', 'password')
        add_user('hannah', 'hannah@email.com', 'password')
        with self.client:
            ...
            self.assertIn(
                'james@email.com', data['data']['users'][0]['email'])
            self.assertTrue(data['data']['users'][0]['active'])
            self.assertFalse(data['data']['users'][0]['admin'])
            self.assertIn('hannah', data['data']['users'][1]['username'])
            self.assertIn(
                'hannah@email.com', data['data']['users'][1]['email'])
    ...
    def test_main_with_users(self):
        ...
        with self.client:
            ...
            self.assertIn(b'hannah', response.data)
